	...
	return (
		<div class="news-listing-item">
			<div class="news-listing-item__top">
				<Link to={`/news-listing/news-desc/?id=${item.id}`}>
...
const NewsListing = () => {
	...
	const filterNewsListingItems = () => {
		...
		if (fromDate !== '' && fromDate !== undefined) {
			filteredNewsListingItems = filteredNewsListingItems.filter((item) => {
				const dateSplit = item.published.split('/');
				return new Date('20' + `20${dateSplit[2]}`.slice(-2), dateSplit[0] - 1, parseInt(dateSplit[1] + 1)).toISOString().split('T')[0] >= fromDate.split('T')[0];
			});
		}
		if (toDate !== '' && toDate !== undefined) {
			filteredNewsListingItems = filteredNewsListingItems.filter((item) => {
				const dateSplit = item.published.split('/');
				return new Date('20' + `20${dateSplit[2]}`.slice(-2), dateSplit[0] - 1, parseInt(dateSplit[1] + 1)).toISOString().split('T')[0] <= toDate.split('T')[0];
			});
		}
	...
	return (
		<>
			...
			<div id="news-listing">{newsListingItemsFiltered.map((item) => getNewsListingItemHTML({ ...item }))}</div>
		</>
	);
};
